...
const sendLogMessage = (level, message) => {
    const notification = {
        jsonrpc: "2.0",
...
for (const envFile of envFiles) {
    if (existsSync(envFile)) {
        ...
        if (result.error) {
            sendLogMessage("warn", `⚠️  Warning: Error loading ${envFile}: ${result.error.message}`);
        }
        else {
...
if (!envLoaded) {
    sendLogMessage("warn", `📁 No environment files found. Checked: ${envFiles.join(", ")}`);
}
// GitBook API client
class GitBookAPIClient {
    ...
    resolveSpaceId(explicitSpaceId) {
    ...
    resolveOrganizationId(explicitOrgId) {
    ...
    async getOrganizations() {
        const response = await this.makeRequest("/orgs");
    ...
    async getSpaces(organizationId) {
        ...
        const response = await this.makeRequest(`/orgs/${orgId}/spaces`);
        return response.items;
    }
    async getSpace(spaceId) {
        return this.makeRequest(`/spaces/${spaceId}`);
    }
    async getSpaceContent(spaceId) {
        return this.makeRequest(`/spaces/${spaceId}/content`);
    }
    // Page operations
    async getPageContent(spaceId, pageId, options) {
    ...
    async getPageByPath(spaceId, pagePath) {
        const encodedPath = encodeURIComponent(pagePath);
        return this.makeRequest(`/spaces/${spaceId}/content/path/${encodedPath}`);
    }
    // Search operations
    async searchContent(spaceId, query) {
        const params = new URLSearchParams({ query });
        return this.makeRequest(`/spaces/${spaceId}/search?${params.toString()}`);
    }
    // File operations
    async getSpaceFiles(spaceId) {
        const response = await this.makeRequest(`/spaces/${spaceId}/files`);
        return response.items;
    }
    async getFile(spaceId, fileId) {
        return this.makeRequest(`/spaces/${spaceId}/files/${fileId}`);
    }
    // Collection operations
    async getCollections(organizationId) {
        const orgId = organizationId || this.organizationId;
        if (!orgId) {
            throw new Error("Organization ID is required to list collections. Provide it via parameter or environment variable.");
        }
        const response = await this.makeRequest(`/orgs/${orgId}/collections`);
        return response.items;
    }
    async getCollection(collectionId) {
        return this.makeRequest(`/collections/${collectionId}`);
    }
    async getCollectionSpaces(collectionId) {
        const response = await this.makeRequest(`/collections/${collectionId}/spaces`);
        return response.items;
    }
}
...
if (!apiToken) {
    sendLogMessage("error", "❌ GITBOOK_API_TOKEN environment variable is required");
    sendLogMessage("error", "💡 Please set your GitBook API token in your .env file");
    sendLogMessage("error", "💡 Example: GITBOOK_API_TOKEN=your_token_here");
    sendLogMessage("error", "💡 Get your token from: https://app.gitbook.com/account/developer");
    sendLogMessage("info", "\n📋 Configuration Summary:");
    if (organizationId) {
        const source = argv.organizationId
            ? "CLI argument"
            : "environment variable";
        sendLogMessage("info", `💡 Organization ID: ${organizationId} (from ${source})`);
    }
    else {
        sendLogMessage("info", "💡 No Organization ID configured");
        sendLogMessage("info", "   • Use --organization-id CLI argument");
        sendLogMessage("info", '   • Add "organization-id: your-org-id" to .github/copilot-instructions.md');
        sendLogMessage("info", "   • Set GITBOOK_ORGANIZATION_ID environment variable");
        sendLogMessage("info", "   • Use list_organizations tool to find your org ID");
    }
    if (defaultSpaceId) {
        const source = argv.spaceId ? "CLI argument" : "environment variable";
        sendLogMessage("info", `💡 Default Space ID: ${defaultSpaceId} (from ${source})`);
    }
    else {
        sendLogMessage("info", "💡 No default Space ID configured");
        sendLogMessage("info", "   • Use --space-id CLI argument");
        sendLogMessage("info", '   • Add "space-id: your-space-id" to .github/copilot-instructions.md');
...
server.tool("list_organizations", "List all GitBook organizations accessible with the current API token", {}, {
...
server.tool("list_spaces", "List all spaces in a GitBook organization, optionally filtered by organization ID", {
    organizationId: z
        .string()
        .optional()
...
server.tool("get_space", "Get detailed information about a specific GitBook space", {
...
server.tool("get_space_content", "Get the complete content structure and page hierarchy of a GitBook space", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space to get content from (uses default space if not provided)"),
}, {
...
server.tool("get_page_content", "Retrieve the content of a specific page from a GitBook space, with options for format and metadata", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space containing the page (uses default space if not provided)"),
    ...
    const pageContent = await gitbookClient.getPageContent(effectiveSpaceId, pageId, Object.keys(options).length > 0 ? options : undefined);
    return {
        content: [
            {
                type: "text",
...
server.tool("get_page_by_path", "Retrieve a page from a GitBook space using its path instead of page ID", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space containing the page (uses default space if not provided)"),
...
server.tool("search_content", "Search for content within a GitBook space using a text query", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space to search in (uses default space if not provided)"),
...
server.tool("get_space_files", "List all files (images, documents, etc.) uploaded to a GitBook space", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space to get files from (uses default space if not provided)"),
}, {
...
server.tool("get_file", "Get detailed information about a specific file uploaded to a GitBook space", {
    spaceId: z
        .string()
        .optional()
        .describe("The ID of the space containing the file (uses default space if not provided)"),
...
// Collection Tools
server.tool("list_collections", "List all collections in a GitBook organization, optionally filtered by organization ID", {
    organizationId: z
        .string()
        .optional()
        .describe("Organization ID to filter collections by (uses default organization if not provided)"),
}, {
...
server.tool("get_collection", "Get detailed information about a specific GitBook collection", {
...
server.tool("get_collection_spaces", "List all spaces that belong to a specific GitBook collection", {
    collectionId: z
        .string()
...
server.resource("collections", new ResourceTemplate("gitbook://collections/{organizationId?}", {
    list: async () => {
        const organizations = await gitbookClient.getOrganizations();
        return {
...
server.prompt("fetch_documentation", "Fetch and analyze GitBook documentation content for specific topics", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID to search in (uses default space if not provided)"),
    topic: z
        .string()
    ...
    includeStructure: z
        .string()
        .optional()
        .describe('Set to "true" to include the overall space structure in the analysis'),
}, (args) => {
...
server.prompt("analyze_content_gaps", "Identify gaps and missing content in documentation", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID to analyze (uses default space if not provided)"),
    comparisonSource: z
        .string()
        .optional()
...
server.prompt("content_audit", "Perform quality audits of documentation content", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID to audit (uses default space if not provided)"),
    auditCriteria: z
        .string()
        .optional()
...
server.prompt("documentation_summary", "Generate comprehensive summaries of GitBook spaces", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID to summarize (uses default space if not provided)"),
    summaryType: z
        .string()
        .optional()
...
server.prompt("content_optimization", "Optimize content for SEO, readability, structure, or performance", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID to optimize (uses default space if not provided)"),
    optimizationType: z
        .string()
...
server.prompt("troubleshooting_assistant", "Diagnose and resolve access, sync, content, and integration issues", {
    spaceId: z
        .string()
        .optional()
        .describe("The GitBook space ID experiencing issues (uses default space if not provided)"),
    issueType: z
        .string()
