...
class GitBookAPIClient {
    ...
    async getFile(spaceId: string, fileId: string): Promise<GitBookFile> {
        return this.makeRequest<GitBookFile>(
    ...
    async getCollection(collectionId: string): Promise<GitBookCollection> {
        return this.makeRequest<GitBookCollection>(
...
server.tool(
    ...
    async ({ organizationId }) => {
        const effectiveOrgId =
            gitbookClient.resolveOrganizationId(organizationId);
...
server.tool(
    ...
    "Get detailed information about a specific GitBook collection",
    {
        collectionId: z
            .string()
            .describe("The ID of the collection to retrieve"),
    },
    {
